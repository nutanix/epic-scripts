...
VMDISK_UUID[0]="4692cce8-a4a7-4c58-9eb4-6b0435ce10bd"
...
ErrCheck() {
  if [[ $? -ne 0 ]]; then
    ERRMSG=$@
    echo "$(date)"
	echo -e "$1 \n\nPlease check on PROXY_HOST_HERE and veeam." | mailx -s "ODB Backup Error: backup-mpe.sh" -a /root/scripts/backup/cron.start-backup.log ADMIN_DL_HERE@domain.org
    echo "ERROR: $ERRMSG"
...
###############################################################################
## Backup - Stage - stop user process
###############################################################################

#echo "$(date) === Stop User Process ==="
#/usr/sbin/fuser -u -k -9 -m ${MP[0]}
...
#ErrCheck "Failed to umount"

#Old code
#/usr/bin/lsof ${MP[0]}
#/usr/sbin/vgremove ${LVM_VG[0]} -y
...
## Backup - pvremove
###############################################################################
#echo "$(date) === pvremove vg ==="
#/usr/sbin/pvremove --force --force -y ${i}
