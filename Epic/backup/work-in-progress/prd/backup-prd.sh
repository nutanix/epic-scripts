...
ErrCheck() {
  if [[ $? -ne 0 ]]; then
    ERRMSG=$@
    echo "$(date)"
    #echo -e "$1 \n\nPlease check on PROXY_HOST_HERE and veeam." \
	#| mailx -s "ODB Backup Error: start-backup.sh" \
	#-a /root/scripts/backup/cron.start-backup.log ADMIN_DL_HERE@domain.org
    echo "ERROR: $ERRMSG"
...
/root/scripts/backup/prd/backup-prd01.sh
sleep 60

echo "Start backup-epic.sh"
#This script only does freeze NOT thaw
/root/scripts/backup/prd/backup-epic.sh
sleep 60

echo "Start backup-jrn.sh"
#This script only does freeze NOT thaw
/root/scripts/backup/prd/backup-jrn.sh
sleep 60
...
/root/scripts/backup/prd/backup-epicfiles.sh
sleep 60
...
#/root/scripts/backup/backup-veeam-prd.sh
